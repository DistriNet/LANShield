...
public class PacketUtil {
	 ...
	 * @return boolean
	 */
	public static boolean isValidTCPChecksum(IPAddress source, IPAddress destination,
	...
	public static byte[] calculateTCPHeaderChecksum(byte[] data, int offset, int tcplength, IPHeader ipHeader){
		// TODO: Optmize getting/setting the TCP pseudo header
		byte[] pseudoTcpHeader = ipHeader.getTcpPseudoHeader((short)tcplength).array();
		int buffersize = tcplength + pseudoTcpHeader.length;
		boolean odd = false;
		if(buffersize % 2 != 0){
			buffersize++;
			odd = true;
	...
	public static String getUDPoutput(IPv4Header ipheader, UDPHeader udp){
		return "\r\nIP Version: " + ipheader.getIpVersion() +
				...
				"\r\nDest: " + ipheader.getDestinationIP().toString() +
						":" + udp.getDestinationPort() +
				"\r\nSrc: " + ipheader.getSourceIP().toString() +
						":" + udp.getSourcePort() +
				"\r\nUDP Length: " + udp.getLength() +
				"\r\nUDP Checksum: " + udp.getChecksum();
	}

	public static String getOutput(IPHeader ipHeader, TCPHeader tcpheader,
								   byte[] packetData) {
		...
		.append("\r\nDest: ").append(ipHeader.getDestinationIP().toString())
				.append(":").append(tcpheader.getDestinationPort())
		.append("\r\nSrc: ").append(ipHeader.getSourceIP().toString())
		...
		if (options != null) {
			for (int i = 0; i < options.length; i++) {
				...
				if (kind == 0 || kind == 1) {
					...
					Log.e(formatTag(PacketUtil.class.getName()), "unknown option: " + kind);
				}
			}
		}
		return false;
