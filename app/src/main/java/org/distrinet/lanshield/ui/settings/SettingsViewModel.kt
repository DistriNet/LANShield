...
class SettingsViewModel @Inject constructor(val dataStore: DataStore<Preferences>) : ViewModel() {
    ...
    val hideMulticastNotifications =
        dataStore.data.map { it[HIDE_MULTICAST_NOTIFICATIONS] ?: false }.distinctUntilChanged()

    val hideDnsNotifications = dataStore.data.map { it[HIDE_DNS_NOTIFICATIONS] ?: false }.distinctUntilChanged()
    ...
    fun onChangeHideMulticastNotifications(enabled: Boolean) {
        writeToDataStore(HIDE_MULTICAST_NOTIFICATIONS, enabled)
    }

    fun onChangeHideDnsNotifications(enabled: Boolean) {
        writeToDataStore(HIDE_DNS_NOTIFICATIONS, enabled)
    }
}